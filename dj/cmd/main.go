...
func main() {
	fx.New(
		...
		fx.Provide(
			...
			fx.Annotate(
				config.Load,
				fx.As(new(sharedconfig.AuthConfiger)),
				fx.As(new(sharedconfig.Configer)),
				fx.As(new(config.PaymentConfiger)),
				fx.As(new(sharedconfig.EventConfiger))),
			...
			fx.Annotate(events.NewPublisher, fx.As(new(events.EventPublisherer))),
		),
