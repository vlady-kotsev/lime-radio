...
import (
	...
	"github.com/vlady-kotsev/lime-radio/transmitter/internal/events"
...
func main() {
	fx.New(
		...
		fx.Provide(
			...
			fx.Annotate(config.Load, fx.As(new(sharedconfig.AuthConfiger)), fx.As(new(sharedconfig.Configer)), fx.As(new(config.RadioConfiger)), fx.As(new(sharedconfig.EventConfiger))),
			...
			fx.Annotate(events.NewSubscriber, fx.As(new(events.EventSubscriberer))),
		),
		...
		fx.Invoke(
			...
			func(subscriber events.EventSubscriberer) {},
		),
	).Run()
}
